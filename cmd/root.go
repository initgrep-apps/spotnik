...
func EnsureAuthenticated(cfg *config.Config, store keychain.TokenStore, tokenBaseURL string) error {
	...
	if expiringSoon {
		...
		if err := api.Refresh(context.Background(), nil, tokenBaseURL, refreshToken, cfg.ClientID, store); err != nil {
...
func CheckAuthState(cfg *config.Config, store keychain.TokenStore) bool {
	...
	if expiringSoon {
		...
		if err := api.Refresh(context.Background(), nil, "", refreshToken, cfg.ClientID, store); err != nil {
			_ = store.Delete()
			return true
		}
	}
