...
## graphify — primary codebase search tool

This project has a knowledge graph at graphify-out/ with god nodes, community structure, and cross-file relationships.

**graphify is the default tool for codebase questions. grep is fallback only.**

When the user types `/graphify`, invoke the `skill` tool with `skill: "graphify"` before doing anything else.

Rules:
- For ANY codebase question ("where is X", "how does Y work", "what calls Z", "trace data flow"), run `graphify query "<question>"` FIRST. Only fall back to grep/Glob if graphify returns nothing useful.
