...
// PaneBorderFor returns the ActiveBorder colour from the theme.
// The per-pane identity colour model has been removed — border colour is
// now determined by focus state (ActiveBorder vs InactiveBorder), not pane slot.
// Kept for backward compatibility with any external callers.
func PaneBorderFor(paneID string, th theme.Theme) lipgloss.Color {
	return th.ActiveBorder()
}
