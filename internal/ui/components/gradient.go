...
// polls), so this uses ClearPendingOnForwardProximity instead of SetConfirmed's
// exact-match check. When the poll position has reached or passed the pending seek
// target (direction-aware), hasPending is cleared and the bar resumes normal polling.
func (b *GradientSeekBar) SetPositionConfirmed(posMs int) {
	b.ClearPendingOnForwardProximity(posMs)
