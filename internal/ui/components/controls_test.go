...
func TestControls_ShuffleOn(t *testing.T) {
	...
	// Shuffle is on so it should be styled differently — we just verify the symbol is present.
	// Color assertions are indirect here since lipgloss wraps chars with ANSI codes.
}
...
func TestControls_RepeatTrack(t *testing.T) {
	...
	output := c.Render()
	assert.Contains(t, output, "🔂", "repeat track should show 🔂 symbol")
}
