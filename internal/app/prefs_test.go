...
func TestAppNew_AppliesSavedVisualizer(t *testing.T) {
	...
	assert.Equal(t, 1, np.VisualizerPattern(), "App.New should apply saved visualizer to NowPlayingPane")
}
...
func TestAppNew_InvalidVisualizer_Wraps(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		...
		np := a.NowPlayingPane()
		require.NotNil(t, np)
		// Must be in [0, patternCount). 99 % 2 = 1
		pat := np.VisualizerPattern()
		assert.True(t, pat >= 0 && pat < 2, "out-of-range visualizer should wrap to valid index, got %d", pat)
	})
}
