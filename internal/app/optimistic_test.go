...
// TestPlaybackCmdPending_SuppressesPollingWrite verifies that a PlaybackStateFetchedMsg
// carrying stale (pre-command) Spotify data is silently discarded while a playback
// command is in-flight (playbackCmdPending > 0), preserving the optimistic store value.
...
// TestPlaybackCmdPending_NeverGoesNegative verifies that receiving a PlaybackCmdSentMsg
// without a prior PlaybackRequestMsg does not panic or decrement playbackCmdPending below zero.
