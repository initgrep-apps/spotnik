...
| 13 | TUI Design System | `features/13-tui-design-system/` | in-progress | 150–172, 183–192 | `internal/uikit` package (18 primitives: PaneChrome, OverlayChrome, Panel, TableChrome, ListRow, LockedRow, SectionLabel, EmptyState, URLBox, HeaderBar, StatusBar, KeyBar, Chip, FormField, Toast, StatusGlyph, ProgressBar, Spinner), frozen glyph catalogue with ascii fallback, role-to-token matrix, `docs/TUI-DESIGN-SYSTEM.md` reference, `⚠`→`◬` swap across cliout + TUI, `ᐅ` removal; 169–172 post-testing regression fixes all merged. **Glyph-fallback follow-up (183–192)** — post-merge audit (`docs/superpowers/specs/2026-04-29-glyph-fallback-audit-design.md`) found `internal/cliout` ignores `ui.glyphs`, `renderGrid` builds `BorderConfig` inline without glyph fields, visualizer is unicode-only, controls/infobox hand-roll borders. 10 stories close the gap end-to-end: catalogue extensions + uikit self-audit (183), cliout integration (184), PaneChrome migration (185), OverlayChrome migration (186), PlaybackControls primitive + nowplaying title (187), Toast bubbleup registration (188), visualizer ASCII renderer (189), search/devices content cleanup (190), remaining inline glyph leaks + ProgressBar (191), CI guards + LANG=C matrix + smoke test (192) |
...
*Last updated: 2026-04-29* (feature 13 reopened: stories 183–192 added for the glyph-fallback audit follow-up; status flipped `done` → `in-progress`; 192 ships the CI guards and `LANG=C` matrix that gate ASCII parity)
