...
| 12 | CLI Output Renderer | `features/12-cli-output/` | done | 146–149 | Reusable `internal/cliout` package, typed message taxonomy, `docs/system/cli.md` reference, palette config, TTY-guarded spinner, validated prompt |
| 13 | TUI Design System | `features/13-tui-design-system/` | done | 150–172, 183–193 | `internal/uikit` package (18 primitives: PaneChrome, OverlayChrome, Panel, TableChrome, ListRow, LockedRow, SectionLabel, EmptyState, URLBox, HeaderBar, StatusBar, KeyBar, Chip, FormField, Toast, StatusGlyph, ProgressBar, Spinner), frozen glyph catalogue with ascii fallback, role-to-token matrix, `docs/system/tui.md` reference, `⚠`→`◬` swap across cliout + TUI, `ᐅ` removal. **Glyph-fallback (183–192):** catalogue extensions + uikit self-audit (183), cliout integration (184), PaneChrome migration (185), OverlayChrome migration (186), PlaybackControls primitive + nowplaying title (187), Toast bubbleup registration (188), visualizer ASCII renderer (189), search/devices content cleanup (190), remaining inline glyph leaks + ProgressBar (191), CI guards (`make check-glyphs`, `scripts/check-banned-glyphs.sh`, `scripts/check-catalogue-leaks.sh`, `scripts/check-render-pane-border.sh`) + LANG=C matrix + smoke test (192). All acceptance criteria met. Last updated: 2026-04-30. |
...
*Last updated: 2026-04-30* (feature 13 closed for the second time: story 193 ships the issues.md follow-up — production hardening, content ASCII fallback gaps, and high-value test depth — bundling 11 sub-threshold items accumulated across stories 183–192)
