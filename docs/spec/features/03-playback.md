...
description: "Shows the currently playing track and gives the user full keyboard-driven control over playback, including play/pause, skip, seek, volume, shuffle, and repeat."
status: done
stories: [03]
---
...
Playback Controls is the core feature of Spotnik -- the one people open the app for. It surfaces what is currently playing on Spotify (track name, artist, album) in the center pane and gives the user full keyboard-driven control over playback: play/pause, next/previous, seek, volume, shuffle, and repeat. The UI must feel rock-solid and responsive, with optimistic updates shown immediately while the next API poll confirms server state.

The feature introduces the tick-based polling architecture that keeps Spotnik in sync with Spotify. A `tea.Tick` loop fires every 1000ms, fetching `GET /me/player` and updating the central Store. Between polls, local progress interpolation increments the seek bar by 1000ms each tick so it moves smoothly without hammering the API. This polling pattern becomes the foundation that later features (queue, devices, stats) build on.

The PlayerPane lives in the center of the grid layout and renders track metadata, a seek bar, transport controls, and a volume bar. All playback state lives in the Store; the pane's `View()` is pure, reading from the Store and rendering styled output using theme tokens. When Spotify returns 204 (nothing playing), the pane renders a clean empty state.

---

## Story: Playback Controls (spec 03)
...
**Design tokens used:** `theme.SeekBar()`, `theme.VolumeBar()`, `theme.PlayingIndicator()`, `theme.TextPrimary()`, `theme.TextSecondary()`, `theme.TextMuted()`. Get the active theme via the `theme Theme` field injected into the pane by `app.New()`.
...
- **Nil state:** When `GET /me/player` returns 204, set `store.PlaybackState = nil` and `store.CurrentTrack = nil`. PlayerPane's `View()` checks for nil and renders the empty state.
- **On 429**: back off for `Retry-After` seconds, show status bar message
- **Progress interpolation**: between polls, increment `progressMs` by 1000ms each tick locally so the seek bar moves smoothly without hammering the API
- **Local interpolation state:** The PlayerPane model holds `localProgressMs int` as pane-local state (not in the Store). It increments by 1000 on each tick when playing, and resets to the server value when `playbackStateFetchedMsg` arrives.

**Seek bar behavior:**
- Width: full pane width minus 4 chars padding (2 each side)
- On `Seek` command: immediately update local `progressMs` to the seeked position, then the next poll confirms the actual server state
- Seeking UI: left/right arrow keys on seek bar move by 10s increments (only when Player pane is focused)
...
> **Focus:** The root model passes a `focused bool` to each pane. Panes only handle key events when focused. The root model sets focus based on the active pane state.
...
1. **API models** -- Define Go structs that map Spotify's playback JSON responses to typed models.
...
2. **Player API calls** -- Implement HTTP client methods for all Spotify playback endpoints.
...
3. **Seek bar component** -- Build a progress bar component that renders elapsed/total time with a visual fill bar.
...
4. **Volume bar component** -- Build a volume indicator component with percentage label.
...
5. **Transport controls component** -- Build the playback controls row showing play/pause, skip, shuffle, and repeat icons.
...
6. **PlayerPane model** -- Implement the central pane's Bubble Tea model that ties together all components and handles keybindings.
...
7. **Polling loop** -- Set up the root model's tick-based polling that keeps playback state in sync with Spotify.
   ...
   - Tests:
     ...
     - `TestPollingLoop_FetchesAndUpdatesStore` (integration) -- tick -> fetch -> playbackStateFetchedMsg -> store updated with new track data

8. **Integration wiring** -- Wire the PlayerPane into the root app model with header/status bar integration.
   ...
   - Acceptance criteria:
     ...
     - Status bar shows player-context keybindings when player pane is focused
   - Tests:
     - `TestApp_PlayerPaneRouting` (integration) -- key events routed to player pane when focused
     - `TestApp_HeaderShowsDevice` (integration) -- device name from playback state appears in header view
