---
title: "Search"
description: "A fast, keyboard-native search overlay that lets users find and play tracks, artists, albums, and playlists without leaving the terminal — press /, type, see live results, and act on them instantly."
status: done
stories: [05]
---
...
Spotnik is a terminal Spotify client for developers who live in the terminal. Search is a core interaction: the user presses `/` from anywhere in the app to open a floating overlay, types a query, and sees live results grouped by tracks, artists, albums, and playlists. Results appear within 400ms of the last keypress thanks to a 300ms debounce mechanism that prevents flooding the Spotify API.

The search overlay layers above the existing three-pane layout without replacing any pane. While open, the background is dimmed and all input is captured by the overlay. The user can navigate results with `j/k`, jump between sections with `Tab`, play a result with `Enter`, add a track to the queue with `a`, or close the overlay with `Esc` to return to exactly where they were. This makes search a zero-friction, keyboard-driven experience that fits naturally into the developer workflow.

Search depends on Feature 02 (Auth) for authenticated API access and can be built in parallel with Feature 04 (Library Browser).

---

## Story: Search Overlay with Live Results (spec 05)
...
1. **Search API** — Implement the Spotify search endpoint wrapper. The function accepts a query, types list, and limit, calls `GET /search`, and returns a parsed `SearchResult` struct containing tracks, artists, albums, and playlists.
...
2. **Debounce logic** — Implement the 300ms debounce mechanism so rapid keypresses do not fire overlapping searches. Each keypress schedules a `searchDebounceMsg`; when the message arrives, it only fires the search if the query has not changed since the tick was scheduled.
...
3. **SearchOverlay model** — Build the `SearchOverlay` Bubble Tea model. It embeds a `bubbles/textinput` for the query, tracks the active section and cursor position within sections, and handles all keybindings defined in the search keymap.
...
4. **Result rendering** — Implement the `View()` rendering for search results. Each section (Tracks, Artists, Albums, Playlists) has a header, items are truncated to fit the overlay width, and the selected item is highlighted using the `SelectedBg()` token.
   - Files: `internal/ui/panes/search.go`
   - Implementation steps:
...
5. **Overlay focus management** — Wire the search overlay into the root app model. `/` opens the overlay and captures all input; `Esc` closes it and restores the previous pane focus. While the overlay is open, the three-pane background is rendered with `lipgloss.NewStyle().Faint(true)`.
