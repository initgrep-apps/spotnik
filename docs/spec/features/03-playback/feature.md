...
- [x] Currently playing track visible within 1s of app launch
- [x] All transport controls respond under 200ms (optimistic update shown immediately)
- [x] Seek bar updates every 1s via local interpolation; gradient renders correctly
- [x] NowPlaying visualizer animates in sync with playback using braille or block chars
- [x] Context-aware playback queues the full source collection when playing any song
- [x] Repeat-one state shows ↻¹ superscript icon; volume shows partial-block bar
- [x] Interactive seek bar with ←/→ debounce, Shift+←/Shift+→ skip track
