...
| `.github/workflows/release.yml` | Remove `RELEASE_PAT` validation step and env var |
...
Remove the `RELEASE_PAT` validation step and the `RELEASE_PAT` env var passed to the
GoReleaser action from `.github/workflows/release.yml` — both were only needed for
cross-repo Homebrew/Scoop pushes. The `GITHUB_TOKEN` built-in is sufficient for
everything that remains. The `RELEASE_PAT` secret can be deleted from the repo settings
(manual step).
...
### Manual
Download a pre-built binary from the [Releases page](https://github.com/initgrep-apps/spotnik/releases/latest).
```
